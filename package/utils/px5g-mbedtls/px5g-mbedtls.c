...
int selfsigned(char **arg)
{
	...
	*/
	mbedtls_asn1_sequence *eku = NULL, *ext_key_usage;
	char *sanval;
	uint8_t ipaddr[16] = { 0 };
	...
	while (*arg && **arg == '-') {
		...
		if (!strcmp(*arg, "-der")) {
			...
			if (!strncmp(arg[1], "extendedKeyUsage=", strlen("extendedKeyUsage="))) {
				...
				*sanval++ = '\0';
				//build sAN list
				if (!strncmp(arg[1], "DNS:", strlen("DNS:"))) {
				...
				} else if (!strncmp(arg[1], "EMAIL:", strlen("EMAIL:"))) {
					san_cur->node.type = MBEDTLS_X509_SAN_RFC822_NAME;
					san_cur->node.san.unstructured_name.p = (unsigned char *) sanval;
					san_cur->node.san.unstructured_name.len = strlen(sanval);
				} else if (!strncmp(arg[1], "IP:", strlen("IP:"))) {
				...
				} else if (!strncmp(arg[1], "URI:", strlen("URI:"))) {
